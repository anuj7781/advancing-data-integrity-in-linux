...
compute()
{
	...
	sync
        ...
        #echo "fio --name=btrfswrite_a --ioengine=io_uring --directory=/mnt --blocksize=${bs} --readwrite=${rw} --filesize=${sze}G --size=${size}G --io_size=${io_size}G --numjobs=${njob} --iodepth=${depth} --randseed=1 --direct=1 --output-format=json -output=$DIR/${ot} --rate_iops=4220 --group_reporting" > $DIR/fio_command_${ot}_a
        #fio --name=btrfswrite_a --ioengine=io_uring --directory=/mnt --blocksize=${bs} --readwrite=${rw} --filesize=${sze}G --size=${size}G --io_size=${io_size}G --numjobs=${njob} --iodepth=${depth} --randseed=1 --direct=1 --output-format=json -output=$DIR/${ot} --rate_iops=4220 --group_reporting
