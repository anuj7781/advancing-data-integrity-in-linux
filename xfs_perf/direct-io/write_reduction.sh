...
compute()
{
	...
	mount -t xfs ${dev} /mnt
        ...
        echo "fio --name=xfs_a --ioengine=io_uring --direct=1 --directory=/mnt --blocksize=${bs} --readwrite=${rw} --filesize=${sze}G --size=${size}G --io_size=${io_size}G --numjobs=${njob} --iodepth=${depth} --randseed=1 -output=$DIR/fio_out_${ot}_a --group_reporting" > $DIR/fio_command_${ot}_a
        fio --name=xfs_a --ioengine=io_uring --direct=1 --directory=/mnt --blocksize=${bs} --readwrite=${rw} --filesize=${sze}G --size=${size}G --io_size=${io_size}G --numjobs=${njob} --iodepth=${depth} --randseed=1 -output=$DIR/fio_out_${ot}_a --group_reporting
